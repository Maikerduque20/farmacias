...
<div class="container py-4">
  <h2 class="text-center mb-4">Farmacias seleccionadas de <?php echo htmlspecialchars($nombre); ?></h2>


 <?php
    // Suponiendo que ya tienes $conexion y $correo definidos y validados
    $queryTratamientos = "SELECT * FROM farmacia WHERE correo = ?";
...
?>
  <?php if ($resultTratamientos->num_rows > 0): ?>
        ...
        <div class="accordion-item">
          ...
          <div id="collapse<?= $index ?>" class="accordion-collapse collapse" data-bs-parent="#accordionTratamientos">
            <div class="accordion-body">
              ...
              <form action="borrar_farmacia.php" method="POST">
    ...
    <?php else: ?>
      <p class="text-center">No tienes Farmacias registradas.</p>
      <?php endif; ?>
...
<div class="modal fade" id="modalTratamiento" tabindex="-1" aria-labelledby="modalLabel" aria-hidden="true">
  <div class="modal-dialog">
    <form method="POST" action="registro_farmacias.php" class="modal-content">
      ...
      <div class="modal-body">
        ...
        <div class="mb-3">
          ...
          <select class="form-control" name="farmacia" required="">
            <option value="" selected disabled>-- Selecciona una farmacia --</option>
                <?php
                $farmacias = [
                    "Farmavida",
                    "Farma Uno",
                    "Farmacia SAN JOSÉ C.A",
                    "Farmacia Ubami",
                    "Biofarma",
                    "Farmacia Páez",
                    "Farmacia El Sarare",
                    "BOTIQUERIA Farmacia Alto Apure",
                    "Pro Farma",
                    "Farmacia Apure",
                    "Farma Medic",
                    "INSUMEDICA VÁSQUEZ F.P",
                    "FARMACIA LA PERIQUERA 0447 C.A"
                ];

                foreach ($farmacias as $nombre) {
                    echo "<option value=\"" . htmlspecialchars($nombre) . "\">$nombre</option>";
                }
                ?>
          </select>
        </div>
